...
Plugin.loadPackageJSON = function(pluginPath) {
  ...
  if (! ( pjson.name && ( pjson.name.indexOf('homebridge-') == 0 || pjson.name.indexOf('@homebridge/homebridge-') == 0))) {
    throw new Error("Plugin " + pluginPath + " does not have a package name that begins with 'homebridge-' or '@homebridge/homebridge-'.");
  }
...
Plugin.getDefaultPaths = function() {
  ...
  for (var index in paths) {
    paths.push(paths[index]+'/@homebridge');
  }
  return paths;
}
